...
**Sample library** (`E6`): after capturing or with the preset loaded, use **SAVE to library** (optional name); clips persist under the app’s private files directory (`sample_library/`). **LOAD from library** restores audio and metadata (pitch, reverse, quantized bar hint when saved) and leaves **PLAY/STOP** loop off until you toggle it. **LOAD WAV** (same submenu) opens the system picker to import a **PCM16 WAV** from storage (SD, Downloads, etc.); after import, **continuous loop** is on and the persisted “last library” restore id is cleared so the next cold start falls back to the preset unless you save/load an internal clip again. Use **Rename** / **Delete** / **favorite** on the selected internal clip; favorites sort to the top (★ prefix). The **last loaded or saved internal** library id is restored on the next app launch when the files still exist.
