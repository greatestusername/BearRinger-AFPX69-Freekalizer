...
- `E3-S1` (P0, done) Implement Main Pitch knob (`-50%..+50%`, pitch-only). (Portable `StreamingCheapPitchShifterMono` **phase-vocoder** in `core`: Hann STFT **2048 / hop 256**, **input ring buffer** so windows are always real audio — **never** zero-pad short callbacks to fft size (that was destroying the PV on typical Android bursts ≈192 frames); while the ring primes, output follows **dry** input; mono sum → stereo duplicate; **FX bus** after sampler+scratch deck + **monitor** when shift active; **hysteresis bypass** at unity; **reset** when leaving bypass or scratch release on FX route; Nyquist zeroed; ratio **slew** (~0.14 s τ) for tape-like glide; optional span ±12/±24/±50; Android `SeekBar` + live label.)
...
- `E4-S1` (P1, done) Implement Filter (LP/HP/BP). (`BiquadMonoFilter` in `core`; Android applies it to `fxBusScratch` **after** master pitch, delay, flanger, and scratch tap — see FX order in `AndroidAudioEngineController`; UI LP/HP/BP buttons in EQ zone.)
...
- `E4-S3` (P1, done) Implement Delay with BPM-based timing. (`DelayBeatMath` + `InterleavedFeedbackDelay` in `core`; FX-bus echo **after** master pitch on the bus (see live chain in `AndroidAudioEngineController`); beat divisions 1/4–4 beats from internal BPM; **send OFF** stops new input while feedback tail decays; UI: send toggle + beats + feedback + wet; tests for beat→frames + line.)
- `E4-S4` (P1, done) Implement Flanger with timing/modulation controls. (`StereoFlanger` variable-delay feed-forward comb in `core`; `FlangerBeatMath` LFO Hz from beats/cycle vs internal BPM; FX order on bus: **master pitch** → delay → **flanger** → filter → tape/reverb (order presets in **MENU FX**); **OFF** bypasses comb while ring/LFO advance; UI: ON toggle + LFO period + base/sweep/manual/wet sliders; tests for LFO math + bypass + engaged delta energy.)
...
- DSP implementation for pitch-only vs pitch+tempo quality/performance tradeoff (**current:** sample pitch = varispeed; main pitch = **2048/256** phase-vocoder + **input ring** (`StreamingCheapPitchShifterMono`), **hysteresis bypass** at unity, **state reset** on resume from bypass or post-scratch, **slower ratio slew** for glide; further upgrade: Rubber Band / native high-quality pitch)
...
- `2026-03-29` - **Master pitch quality / correctness:** Fixed gross “noise” artifacts from **stale phase-vocoder state** when resuming after **unity bypass** (dry path did not advance the STFT FIFO) or after **scratch** (FX route skips PV while touching). Added **hysteresis** around ratio≈1 so tiny slew/wow does not flip bypass every buffer. `StreamingCheapPitchShifterMono`: **Nyquist bin zeroed** on shifted frames, slightly **lower magnitude EMA** (0.78) for clearer transients; added `StreamingCheapPitchShifterMonoTest`. **Docs:** E3-S1/E4 FX-order notes aligned with `AndroidAudioEngineController` (master pitch before delay/flanger/filter on the FX bus). `./gradlew :core:test` + `:app:assembleDebug` verified.
- `2026-03-29` - **Master pitch — real STFT streaming fix:** Root cause of “not pitch shifting / garbage” was **zero-filling** analysis windows when `framesPerBurst` (e.g. 192) **< fftSize** (was 4096), which invalidates the phase vocoder every callback. **Fix:** internal **64k-sample input ring**; form STFT frames only from **contiguous real input**, advance by `hop` per frame; **dry passthrough** while the ring primes; device STFT **2048/256**; magnitude EMA **0.72**; `MAIN_PITCH_SLEW_TAU_SEC` **0.14 s** for audible glide; chunked regression test `chunkedInput_smallerThanFft_staysFiniteAndEventuallyDiffersFromDry`.
</think>
